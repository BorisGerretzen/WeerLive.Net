...
{
    [Test]
    public async Task GetAsync_DemoAmsterdam()
    {
        const string demoResponse = """
                                    {
                                        "liveweer": [
                                            {
                                                "plaats": "Amsterdam",
    ...
    [Test]
    public void GetAsync_InvalidLocation()
    {
        const string mockResponse = """
                                    { "liveweer": [{"fout": "Met de demo-key kun je alleen weergegevens van Amsterdam opvragen. Vraag een API-key aan voor de weergegevens van plaatsen in heel Nederland."}]}
                                    """;
        var httpClient = MockClient("?key=demo&locatie=Anders", mockResponse);
        var options = Options.Create(new WeerLiveOptions { ApiKey = "demo" });

        var client = new WeerLiveClient(httpClient, options);

        Assert.ThrowsAsync<WeerLiveException>(async () => await client.GetAsync("Anders"));
    }
